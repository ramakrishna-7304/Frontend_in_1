...
export default function Navbar() {
	...
	useEffect(() => {
		const onScroll = () => {
			...
			// Active section detection only on home route
		...
		<header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'backdrop-blur-md bg-[#0F172A]/70 shadow-lg' : 'bg-transparent'} animate-fade-in-up`}>
			<nav className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
				<div className="flex items-center gap-3">
					<button onClick={() => goToSection('home')} className="text-2xl font-extrabold tracking-tight">
				...
				<div className="hidden md:flex items-center gap-8">
					...
					<button onClick={() => goToSection('contact')} className="px-5 py-2.5 rounded-full bg-gradient-to-r from-primary to-secondary text-white font-semibold shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-transform hover:scale-105 focus-visible:scale-105">
						Get Started
					</button>
				</div>
			...
			<div className={`md:hidden transition-all duration-300 overflow-hidden ${open ? 'max-h-96' : 'max-h-0'}`}>
				<div className="px-4 pb-4 space-y-2">
					{links.map((l) => (
						<button key={l.id} onClick={() => goToSection(l.id)} className={`block w-full text-left px-3 py-2 rounded-lg hover:bg-white/5 ${active === l.id ? 'text-white' : 'text-white/80'}`}>
							{l.label}
						</button>
					))}
					<button onClick={() => goToSection('contact')} className="w-full px-4 py-2.5 rounded-xl bg-gradient-to-r from-primary to-secondary font-semibold shadow-lg">
						Get Started
					</button>
				</div>
			</div>
